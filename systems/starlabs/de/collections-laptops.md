Built for Linux. Not adapted to it.

Choose by how you work: convertible, privacy-conscious, or full-fat performance.

[Compare full specs](#laptops-compare)

[](/pages/starlite)

[StarLite](/pages/starlite)

Fanless, low-power Linux tablet for flexible private work.

From €13,37

Display 12.5-inch 3:2 touchscreen

Weight 0.9 kg tablet / 1.6 kg with keyboard

Power class Fanless 7W tablet-classIntel N350 + 16GB LPDDR5X

Battery Up to 12 hrs

Built for flexible privacy

No fan noise, pen-ready touchscreen flexibility, and a full Linux desktop when docked with the keyboard.

[Product Overview](/pages/starlite) [Configure](/products/starlite)

[](/pages/starbook)

[StarBook Horizon](/pages/starbook)

StarBook Horizon Plus with Intel Core i5-1334U, 52Wh battery, and haptic trackpad.

From €13,37

Display 13.4-inch 3:2 90Hz

Weight 1.1 kg

Power class Active-cooled 15W laptop-classIntel i5-1334U + 32GB LPDDR5X

Battery 52Wh

Built for everyday headroom

Everything in StarBook Horizon, plus higher CPU headroom, PCIe Gen 4 SSD support, haptic input, and wider keyboard layout options.

[Product Overview](/pages/starbook) [Configure](/products/starbook)

[](/pages/starfighter)

[StarFighter](/pages/starfighter)

Premium Linux workstation hardware with haptic trackpad, removable camera/mic, and a serious Ultra upgrade path.

From €13,37

Display 16-inch 16:10, QHD 165Hz or 4K 120Hz

Weight 1.6 kg

Platform Intel Ultra or AMD Ryzen AIUp to Intel 285H + 64GB LPDDR5X

Battery Up to 18 hrs

Built as a workstation

AZ91D magnesium, haptic trackpad, detachable camera/microphone, and Ultra adds 4K 120Hz, 64GB memory, Core Ultra 9 headroom, and Encrypted Memory (TME).

[Product Overview](/pages/starfighter) [Configure](/products/starfighter)

 Built for ownership, updates, and long-term use.

Linux-first

Designed around Linux, not treated as an afterthought.

Open firmware

 Coreboot on supported models, with configurable firmware options.

Built for ownership

 Serviceable parts and supported upgrade paths designed for longer hardware life.

Direct support

UK-based support from people who know the hardware.

Compare full specs

See the key differences at a glance.

|   Model    |                                 StarLite                                  |                                            StarBook Horizon  StarBook Horizon Plus                                            |                            StarFighter  StarFighter Ultra  StarFighter AMD                            |
|------------|---------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|
|  Summary   |         Fanless, low-power Linux tablet for flexible private work.        |     Everything in StarBook Horizon, plus higher CPU headroom, a larger battery, haptic input, and wider keyboard layouts.     |             Everything in StarFighter, plus 4K 120Hz, 64GB memory, and long-term headroom.            |
|  Display   |                         12.5-inch 3:2 touchscreen                         |                                                       13.4-inch 3:2 90Hz                                                      |                              16-inch 16:10, 4K 120Hz, VRR 48-120Hz, DRRS                              |
|Form factor |                        Tablet + magnetic keyboard                         |                                                    Light clamshell portable                                                   |                                    Maximum-spec mobile workstation                                    |
|   Weight   |                    0.85 kg tablet, 1.5 kg with keyboard                   |                                                             1.1 kg                                                            |                                                 1.6 kg                                                |
|Power class |                      Fanless 7W tablet-class platform                     |                                               Intel Core i5-1334U + 32GB LPDDR5X                                              |                           Core Ultra 9, 64GB LPDDR5X, Encrypted Memory (TME)                          |
|   USB-C    |                 2x USB-C with charging and display support                |                                                  1x Thunderbolt 4 + 1x USB-C                                                  |                           2x Thunderbolt 4 with charging and display support                          |
|  Battery   |                               Up to 12 hrs                                |                                                              52Wh                                                             |                                              Up to 18 hrs                                             |
|Why this one| No fan noise, pen-ready touchscreen flexibility, and a full Linux desktop.| Everything in StarBook Horizon, plus PCIe Gen 4 SSD support, haptic trackpad, 52Wh battery, and wider keyboard layout options.| Everything in Standard, plus 4K 120Hz, 64GB memory, Core Ultra 9 headroom, and Encrypted Memory (TME).|
|  Show more |                                                                           |                                                                                                                               |                                                                                                       |

 Last-generation StarBook — limited stock

[](/pages/starbook-ultra)

[StarBook Mk VII Ultra](/pages/starbook-ultra)

Last-generation StarBook clearance. Limited stock remaining.

From €13,37

Available keyboard layouts: French, German

14-inch 4K matte display

Up to 96GB memory

Around 1.4 kg

Final Mk VII stock

[Product Overview](/pages/starbook-ultra) [Configure](/products/starbook-ultra)
