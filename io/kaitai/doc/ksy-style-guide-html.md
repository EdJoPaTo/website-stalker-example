...
One MUST use the canonical (exact) spelling of the encoding name. The match is**case-sensitive**: for example, `UTF-8` is correct, but `utf-8` or `Utf-8` will
trigger a warning.

Note

 KS canonical encoding names are mostly based on the[IANA Character Sets registry](https://www.iana.org/assignments/character-sets/character-sets.xhtml) ("Preferred MIME Name" column where available,
otherwise "Name"), with a few deliberate exceptions (like `ASCII` instead of`US-ASCII`). IANA also states that encoding names are case-insensitive — KS,
...
`UTF-16` and `UTF-32` (without an explicit byte order suffix) are deliberately**not** accepted. One MUST use the byte-order-explicit variants instead:
