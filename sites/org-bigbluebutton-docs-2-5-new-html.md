...
All the presentations are sent via payload (the body of the request) such as when preuploading documents in `/create` endpoint, whether by a link to download the file or by `base64` encoded file. See the [API documentation](https://docs.bigbluebutton.org/dev/api.html#insertdocument) for more information
