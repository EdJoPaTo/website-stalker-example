...
BigBlueButton uses a TURN server for NAT traversal. By default, a STUN server from freeswitch.org is configured for BigBlueButton. You can [set up your own TURN/STUN server](https://docs.bigbluebutton.org/admin/setup-turn-server.html), and configure it in `/usr/share/bbb-web/WEB-INF/classes/spring/turn-stun-servers.xml`.
...
Greenlight can be patched to allow more fine-grained control of recording visibility and publications. With [https://github.com/ichdasich/greenlight/tree/rec\_restrictions](https://github.com/ichdasich/greenlight/tree/rec_restrictions) a third value is added to the recording visibility pull-down menu (public/unlisted/private), which leads to corresponding recording metadata being set (gl-listed=true/unlisted/false) in the recording either in scalelite or on the BigBlueButton host.
...
`{"log":"I, [] INFO -- : [] Redirected to https:///bigbluebutton/api/join?meetingID=\u0026fullName=\u0026password=... }`
