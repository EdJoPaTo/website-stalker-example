...
We recommend installing BigBlueButton with a ‘clean’ and dedicated Ubuntu 18.04 64-bit server with no prior software installed. If you want to upgrade from BigBlueButton 2.4, you can upgrade your existing server using the `bbb-install-2.5.sh` script and passing `bionic-250` instead of `bionic-240` as a repository. If you want to upgrade from an earlier version of BigBlueButton like 2.2, we recommend setting up a clean server for BigBlueButton 2.5 and, after setup, [migrate over your existing recordings](https://docs.bigbluebutton.org/admin/customize.html#transfer-published-recordings-from-another-server).
