...
Whenever you upgrade a server to the latest version of BigBlueButton, either using the [manual upgrade steps](https://docs.bigbluebutton.org/2.2/install.html#upgrading-from-bigbluebutton-22) or the [bbb-install.sh](https://github.com/bigbluebutton/bbb-install) script, if you have made custom changes to BigBlueButton’s configuration files, the packaging scripts may overwrite these changes.
...
* use the helper function `enableUFWRules` to [restrict access to specific ports](https://docs.bigbluebutton.org/admin/customize.html#restrict-access-to-specific-ports), and
...
When a meeting finishes, the BigBlueButton server [archives the meeting data](https://docs.bigbluebutton.org/dev/recording.html#archive) (referred to as the “raw” data).
...
This change will cause BigBlueButton to generate an additional `.mp4` file for the video components (webcam and screen share) that was shared during the session. This change only applies to new recordings. If you want this change to apply to any existing recordings, you need use the `bbb-record` command to [rebuild them](https://docs.bigbluebutton.org/dev/recording.html#rebuild-a-recording).
...
To [always record every meeting](https://docs.bigbluebutton.org/admin/customize.html#always-record-every-meeting), add the following to `apply-config.sh`.
...
where `` is the the file name of the raw recording in `/var/bigbluebutton/recording/raw`, such as
...
Warning: If you have a large number of recordings, this will rebuild *all* of them, and not process any new recordings until the rebuild process finishes. Do not do this unless this is you intent. Do not do this command to troubleshoot recording errors, instead see [Recording Troubleshooting](https://docs.bigbluebutton.org/dev/recording.html#troubleshooting).
...
If you expect users to share many webcams, you can [reduce bandwidth for webcams](https://docs.bigbluebutton.org/admin/customize.html#reduce-bandwidth-from-webcams).
...
Thus, we recommend you [enable multiple Kurento](https://docs.bigbluebutton.org/admin/customize.html#run-three-parallel-kurento-media-servers) servers, thereby having one Kurento server for webcams, one for screen share, and one for listen only streams. The settings apply to each Kurento server, so in the above example each Kurento server would have a maximum of 1000 media streams.
...
To use your own background images copy them into the directory`/usr/share/meteor/bundle/programs/web.browser/app/resources/images/virtual-backgrounds`.
For each image copy a thumbnail of the image of 50x50 pixels size into`/usr/share/meteor/bundle/programs/web.browser/app/resources/images/virtual-backgrounds/thumbnails`.
...
```
     name="cdquality">
       ...
       name="interval" value="20"/>
       name="energy-level" value="100"/>

       name="muted-sound" value="conference/conf-muted.wav"/>
       name="unmuted-sound" value="conference/conf-unmuted.wav"/>
       name="alone-sound" value="conference/conf-alone.wav"/>

       name="kicked-sound" value="conference/conf-kicked.wav"/>
       name="locked-sound" value="conference/conf-locked.wav"/>
       name="is-locked-sound" value="conference/conf-is-locked.wav"/>
       name="is-unlocked-sound" value="conference/conf-is-unlocked.wav"/>
       name="pin-sound" value="conference/conf-pin.wav"/>
       name="bad-pin-sound" value="conference/conf-bad-pin.wav"/>
       name="caller-id-name" value="$${outbound_caller_name}"/>
       name="caller-id-number" value="$${outbound_caller_id}"/>
...
```
       name="moh-sound" value="$${hold_music}"/>
...
```
   cmd="set" data="hold_music=local_stream://moh"/>
...
```
   cmd="set" data="hold_music=/opt/freeswitch/share/freeswitch/sounds/en/us/callie/ivr/48000/ivr-to_listen_to_moh.wav" />
...
```
   ...
   name="ANY-NAME-FOR-YOUR-PROVIDER">
     name="proxy" value="sip.example.net"/>
     name="username" value="PROVIDER-ACCOUNT"/>
     name="password" value="PROVIDER-PASSWORD"/>
     name="extension" value="EXTERNALDID"/>
...
```
 name="from_my_provider">
  field="destination_number" expression="^EXTERNALDID">
    application="start_dtmf" />
    application="answer"/>
    application="sleep" data="1000"/>
    application="play_and_get_digits" data="5 7 3 30000 # conference/conf-pin.wav ivr/ivr-that_was_an_invalid_entry.wav pin \d+"/>

    application="transfer" data="SEND_TO_CONFERENCE XML public"/>

 name="check_if_conference_active">
  field="${conference ${pin} list}" expression="/sofia/g" />
  field="destination_number" expression="^SEND_TO_CONFERENCE$">
    application="set" data="bbb_authorized=true"/>
    application="transfer" data="${pin} XML default"/>

 name="conf_bad_pin">
  field="${pin}" expression="^\d{5}$">
    application="answer"/>
    application="sleep" data="1000"/>
    application="play_and_get_digits" data="5 7 3 30000 # conference/conf-bad-pin.wav ivr/ivr-that_was_an_invalid_entry.wav pin \d+"/>
    application="transfer" data="SEND_TO_CONFERENCE XML public"/>
...
```
defaultWelcomeMessageFooter=To join this meeting by phone, dial:  %%DIALNUM%%Then enter %%CONFNUM%%# as the conference PIN number.
...
If you want to enable, edit `/opt/freeswitch/conf/autoload_configs/conference.conf.xml`, look for the block ``, and change the value for `comfort-noise`. You can specify a level of noise, such as 0 (no noise), 350, 1400, etc. Try different values to get the level you desire.

```
 name="comfort-noise" value="1400"/>
...
If you want to specify the default presentation for a given meeting, you can also pass a URL to the presentation as part of the [create](https://docs.bigbluebutton.org/dev/api.html#pre-upload-slides) meeting API call.
...
For more information see [Installing Greenlight](https://docs.bigbluebutton.org/greenlight/gl-install.html).
...
BigBlueButton comes with a [UFW](https://launchpad.net/ufw) based ruleset. It it can be applied on restart (c.f. [Automatically apply configuration changes on restart](https://docs.bigbluebutton.org/admin/customize.html#automatically-apply-configuration-changes-on-restart)) and restricts access only to the following needed ports:
...
Note: if you have configured `sshd` (the OpenSSH daemon) to use a different port than 22, then before running the commands below, change `ufw allow OpenSSH` to `ufw allow /tcp` where `` is the port in use by `sshd`. You can see the listening port for `sshd` using the command `# netstat -antp | grep sshd`. Here the command shows `sshd` listening to the standard port 22.
...
To restrict external access minimal needed ports for BigBlueButton (with [HTML5 client set as default](https://docs.bigbluebutton.org/admin/customize.html#make-the-html5-client-default)). BigBlueButton supplies a helper function that you can call in `/etc/bigbluebutton/bbb-conf/apply-conf.sh` to setup a minimal firewall (see [Setup Firewall](https://docs.bigbluebutton.org/admin/customize.html#setup-firewall).
...
```
     ...
     name="rtp-start-port" value="16384"/>
     name="rtp-end-port" value="24576"/>
...
If always want a specific TURN server configuration, the following to [apply-config.sh](https://docs.bigbluebutton.org/admin/customize.html#apply-confsh) and modify `aaa.bbb.ccc.ddd` and `secret` with your values.
...
When a front-end creates a BigBlueButton session, it may also pass a `welcome` parameter in the [create](https://docs.bigbluebutton.org/dev/api.html#create) API call.
...
![](https://docs.bigbluebutton.org/images/html5-lock-webcam.png)
...
Set `displayBrandingArea: true` in `settings.yml`, restart BigBlueButton server with `sudo bbb-conf --restart` and pass `logo=` in Custom parameters when creating the meeting.
...
Step-by-step instructions for how to configure logs from clients to be logged in a server log file are located in [Administration -\> Configuration Files](https://docs.bigbluebutton.org/admin/configuration-files.html#logs-sent-directly-from-the-client)
...
For information on cofiguring bbb-webhooks, see [bbb-webhooks](https://docs.bigbluebutton.org/dev/webhooks.html).
